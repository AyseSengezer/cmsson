...
class Galleries extends CI_Controller
{
    ...
    public function upload_form($id)
    {
        ...
        if ($item->gallery_type == "image") { // gallery_model den item in gallery_type i

            $viewData->items = $this->image_model->get_all( // buradaki $item gallery_type i image olan item
                array(
                    "gallery_id"    => $id
                ),
                "rank ASC"
            );
            $viewData->folder_name = $item->folder_name; //buradaki $item gallery_model den çektiğimiz veriler
    ...
    public function file_upload($gallery_id, $gallery_type, $folderName)
    {
        ...
        if ($gallery_type == "image") {
            ...
            $image_350x216 = upload_picture($_FILES["file"]["tmp_name"], "uploads/$this->viewFolder/images/$folderName/", 364, 216, $file_name);
    ...
    public function fileDelete($id, $parent_id, $gallery_type, $folder_name)
    {
        ...
        // TODO Alert Sistemi Eklenecek...
        if ($delete) {
            unlink("uploads/$this->viewFolder/images/$folder_name/252x156/$fileName->url");
            unlink("uploads/$this->viewFolder/images/$folder_name/364x216/$fileName->url");
            unlink("uploads/$this->viewFolder/images/$folder_name/851x606/$fileName->url");
